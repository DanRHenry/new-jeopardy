...
export function openTeacherSideWebsocket(className, email, categoriesArray) {
    ...
    socket.addEventListener("message", function (message) {
        ...
        if (!studentList.includes(data.studentEmail)) {
          ...
          if (!document.getElementById("beginGameBtn")) {
            ...
            beginGameBtn.addEventListener("click", () => {
              socket.send(JSON.stringify({ cueToStart: "true" }));
              beginGame(categoriesArray, socket);
            });
      ...
      if (data.showResponse) {
        console.log("showing response: ")
         document.getElementById("promptText").innerText = `${data.activePrompt}: ${data.activeResponse}`
        setTimeout(() => {
            document.getElementById("promptResponseWindow")?.remove()
            document.getElementById("backgroundOverlay")?.remove()
            sessionStorage.removeItem("failedGuess")
